...
const generatedUserId = Math.floor((Math.random() * 10) + 1);
const generatedPINCode = Math.floor((Math.random() * 1000) + 1000);
let bankBalance = Math.floor(Math.random() * 100000);
console.log("========== generatedUserId", generatedUserId);
console.log("========== generatedPINCode", generatedPINCode);
...
const handleDepositingMoney = (amount) => {
    if (amount) {
...
const handleTransferringFunds = (IBAN, transferAmount) => {
    if (IBAN) {
        if (transferAmount) {
...
if (generatedUserId === userID && generatedPINCode === Number(userPIN)) {
    const selectService = await inquirer.prompt([
        {
            type: "list",
